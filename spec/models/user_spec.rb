...
describe User do
 ...
 describe "cuando el email no esta presente" do
  ...
  describe "cuando el fomato del mail es invalido" do
  	it "debe ser invalido" do
  		direcciones = %w[user@prueba,com usuario_usuario_.org usuario@prueba.comusuario@prueba.com usuario@prueba+prueba.com]
  		direcciones.each do |direcciones_invalida|
  			@user.email= direcciones_invalida
  ...
  describe "cuando el formato es valido" do 
  	it "debe ser invalido" do
  		...
  		direcciones.each do |direcciones_valida|
  			@user.email= direcciones_valida
  ...
  describe "cuando el password no esta presente" do
	...
	describe "cuando el password no es igual a la confimacion" do
  	before { @user.password_confirmation ="missmatch"}
  	it { should_not be_valid}
  end
  describe "cuando un psassword es muy corto" do
  	before { @user.password = @user.password_confirmation = "a" * 5 }
  	it { should be_invalid }
  end
  describe "devolve el valor  del metodo de autentidicacion" do
  	before { @user.save }
